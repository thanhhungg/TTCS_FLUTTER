...
import '../Medicine/request_album_assets.dart';
import '../Medicine/request_albums.dart';
import 'avatar_image.dart';

class SelectedImage extends StatefulWidget {
  SelectedImage({super.key, required this.id});
  final int id;
  @override
...
class _SelectedImageState extends State<SelectedImage> {
  ...
  // final List<AssetEntity> selectedAvatar = [];
                  ...
                  childrenDelegate: SliverChildBuilderDelegate(
                    (context, index) {
                      ...
                      return GestureDetector(
                        onTap: () {
                          if (selectedAvatar.contains(entity)) {
                            setState(() {
                              selectedAvatar.remove(entity);
                            });
                          } else {
                            setState(() {
                              selectedAvatar.add(entity);
                            });
                          }
                        },
                        child: Padding(
                          padding: const EdgeInsets.all(1),
                          child: assetMediaWidget(entity, selectedAvatar),
                        ),
                      );
                    },
  ...
  Widget assetMediaWidget(entity, selectedAvatar) => Stack(
        children: [
          Positioned.fill(
